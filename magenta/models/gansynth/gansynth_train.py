...
def init_data_normalizer(config: Flags):
  """Initializes data normalizer."""

    # NOTE: √, lib/model set default to 'specgrams_prespecified_normalizer'
    normalizer = data_normalizer.registry[config['data_normalizer']](config)
    if normalizer.exists():
        return

    # NOTE: √, lib/model set default to 0
    # The Task ID. This value is used when training with multiple workers to
    # identify each worker
    if config['task'] == 0:
        tf.reset_default_graph()
        # NOTE: √, config/mel_prog_hires set default to 'mel', so does lib/model
...
def main(unused_argv):
    ...
    absl.flags.FLAGS.alsologtostderr = True  # also log to std err
    ...
    # Config hparams
    if FLAGS.config:
      # NOTE: √, --config=mel_prog_hires, now only have this, can be set or not.
      config_module = importlib.import_module('magenta.models.gansynth.configs.{}'.format(FLAGS.config))
      # NOTE, √, 
      # - Training
      # - Network
      # - Loss Functions
    ...
    # NOTE: √, e.g. --hparams='{"train_data_path":"/path/to/nsynth-train.tfrecord"}'

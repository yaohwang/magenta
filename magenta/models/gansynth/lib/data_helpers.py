...
class DataHelper(object):
    ...
    def __init__(self, config: Flags):
        self._config = config
        self._dataset_name = config['dataset_name'] # NOTE: √, both config/mel_pro_hires and lib/model set to 'nsynth_tfds'

        self.dataset = datasets.registry[self._dataset_name](config) # NOTE: √, init, record config only
        self.specgrams_helper = self.make_specgrams_helper()


    def provide_data(self, batch_size: int):
        # NOTE: √, main
...
class DataSTFTHelper(DataHelper):
  """A data helper for Linear Spectrograms."""
    ...
    def make_specgrams_helper(self):
        # NOTE: by default DataMelHelper
        final_resolutions = train_util.make_resolution_schedule(**self._config).final_resolutions
...
class DataWaveHelper(DataSTFTHelper):
    ...
    def make_specgrams_helper(self):
        return SpecgramsHelper(
            ...
            spec_shape = (256, 512),
            overlap = 0.75,
            sample_rate = self._config['sample_rate'], # NOTE: lib/model set default to 16000
            mel_downscale = 2
        )

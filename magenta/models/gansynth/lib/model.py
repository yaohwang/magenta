...
from magenta.models.gansynth.lib.flags import Flags
    ...
    def __init__(self, stage_id, batch_size, config):
        ...
        if config['train_time_limit'] is not None:
          ...
          for i in range(1, num_stages):
            stage_times[i] = (stage_times[i-1] * config['train_time_stage_multiplier'])
        ...
        if config['train_progressive']:
            if config['train_time_limit'] is not None:
                ...
                progress = networks.compute_progress(
                    current_image_id,
                    config['stable_stage_num_images'],
                    config['transition_stage_num_images'],
                    num_blocks
                )
        else:
        ...
        # TODO:
        real_images = networks.blend_images(real_images, progress, resolution_schedule, num_blocks=num_blocks)
        ...
        with tf.name_scope('losses'):
            ...
            # AC losses.
            fake_ac_loss = _compute_ac_loss(gan_model.generated_data, gen_one_hot_labels)
            real_ac_loss = _compute_ac_loss(gan_model.real_data, real_one_hot_labels)

            # GL losses.
            is_fourier = isinstance(data_helper, (data_helpers.DataSTFTHelper, data_helpers.DataSTFTNoIFreqHelper, data_helpers.DataMelHelper))
            ...
            if is_fourier \
            and gen_gl_consistency_loss_weight > 0 \
            and stage_id == train_util.get_total_num_stages(**config) - 1:
              wx_fake_gl_loss = fake_gl_loss * gen_gl_consistency_loss_weight

            # Update the loss functions
            gan_loss = gan_loss._replace(
                generator_loss = (gan_loss.generator_loss + wx_fake_ac_loss + wx_fake_gl_loss),
        ...
        ########## Define train ops.
        gan_train_ops, optimizer_var_list = train_util.define_train_ops(gan_model, gan_loss, **config)
        ...
        gan_train_ops, generator_vars_to_restore = train_util.add_generator_smoothing_ops(generator_ema, gan_model, gan_train_ops)

        load_scope = tf.variable_scope(
            gan_model.generator_scope,
            reuse = True,
            custom_getter = train_util.make_var_scope_custom_getter_for_ema(generator_ema)
        )
        ...
        noises_ph = tf.placeholder(tf.float32, [batch_size, config['latent_vector_size']])

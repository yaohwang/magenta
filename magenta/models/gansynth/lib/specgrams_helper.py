...
class SpecgramsHelper(object):
    ...
    def __init__(
        ...
        self._sample_rate = sample_rate  # TODO:
        self._mel_downscale = mel_downscale # TODO:
    ...
    def _get_nfft_nhop(self):
        ...
        if not is_power_2:
            raise ValueError('Wrong spec_shape. Number of frequency bins must be a power of 2, not %d' % n_freq_bins)

        # TODO: suppose to be window size and overlap
        nfft = n_freq_bins * 2
    ...
    def _get_padding(self):
        ...
        if n_samps_inv < self._audio_length:
          raise ValueError('Wrong audio length. Number of ISTFT samples, %d, should be less than audio lengeth %d' % self._audio_length)
